...
pub mod payout_policy {
    ...
    pub fn initialize(
        ...
        config.w_scroll = 1_000;
        config.w_click = 2_000;
        config.w_focus = 3_000;
        config.max_ctx_multiplier = 3_000;
        ...
        msg!(
            "PayoutPolicy initialized | p_base: {} | take_rate: {}%",
            p_base,
            take_rate_bps / 100
        );
        Ok(())
    ...
    pub fn calculate_and_pay(
        ...
        msg!(
            ...
            user_share,
            platform_share,
            admin_cfg.current_epoch,
            count
        );
        Ok(())
    }

    pub fn advance_epoch(ctx: Context<AdvanceEpoch>, new_take_rate_bps: u32) -> Result<()> {
        require!(new_take_rate_bps <= 10_000, ErrorCode::InvalidConfig);
        let admin_cfg = &mut ctx.accounts.admin_config;
        admin_cfg.current_epoch += 1;
        admin_cfg.take_rate_bps = new_take_rate_bps;
        msg!(
            "Advanced to epoch {} | take_rate: {}%",
            admin_cfg.current_epoch,
            new_take_rate_bps / 100
        );
        Ok(())
    }
}
...
pub struct Initialize<'info> {
    ...
    /// CHECK: endereço da plataforma, sem verificação adicional necessária
...
pub struct CalculateAndPay<'info> {
    ...
    #[account(
        ...
        seeds = [
            b"epoch_count",
            user.key().as_ref(),
            &admin_config.current_epoch.to_le_bytes()
        ],
        bump
    )]
    pub epoch_count: Account<'info, EpochCount>,
    /// CHECK: treasury PDA controlado pelo programa
    ...
    #[account(
        mut,
        associated_token::mint = mint,
        associated_token::authority = config.platform
    )]
