...
pub mod reputation_proof {
    ...
    pub fn register_commitment(
        ...
        msg!(
            "Commitment registered | user: {} | epoch: {}",
...
pub struct RegisterCommitment<'info> {
    /// CHECK: usuário sendo rastreado, sem modificação de estado
...
pub struct VerifyAndScore<'info> {
    ...
    /// CHECK: usuário cuja reputação está sendo verificada
    pub user: AccountInfo<'info>,
    pub system_program: Program<'info, System>,
}
...
pub struct CheckTrusted<'info> {
    ...
    pub reputation: Account<'info, Reputation>,
    /// CHECK: usuário sendo consultado
    pub user: AccountInfo<'info>,
    #[account(
