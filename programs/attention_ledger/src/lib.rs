...
declare_id!("AttnLedg111111111111111111111111111111111111");
...
pub struct CreditRecord {
    pub amount: u64,
    pub epoch: u32,
    pub attention_type: u8, // 0=scroll, 1=click, 2=focus
    pub timestamp: i64,
}
...
pub mod attention_ledger {
    ...
    pub fn credit(
        ...
        balance.total = balance.total.checked_add(amount).ok_or(ErrorCode::Overflow)?;
        balance.epoch_total = balance.epoch_total.checked_add(amount).ok_or(ErrorCode::Overflow)?;
        balance.event_count += 1;

        msg!("Credit | user: {} | amount: {} | epoch: {}", balance.owner, amount, epoch);
        Ok(())
    }

    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        let balance = &mut ctx.accounts.user_balance;
        require!(balance.owner == ctx.accounts.user.key(), ErrorCode::Unauthorized);
